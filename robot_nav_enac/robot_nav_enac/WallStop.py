...
class WallStop(NavigationType):
    def __init__(self, logger, get_motor_speed, speed = 0.2, fixed_obstacles = None):
    ...
    def update_odom(self, callback_speed:Callable[[float, float], None], position:OdomData, speed:OdomData, dt=0.0):
        if self.get_motor_speed() >= self.epsilon_min_speed: # * 2 for margin
        ...
        elif self.started_moving_forward and not self.reached_wall and speed.x <= self.epsilon_move_from_coder:
