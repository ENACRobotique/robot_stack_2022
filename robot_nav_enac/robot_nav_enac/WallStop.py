...
class WallStop(NavigationType):
    def __init__(self, logger, get_coder_linear_speed, speed = 0.2, fixed_obstacles = None):
        ...
        self.speed = speed
        self.get_coder_linear_speed = get_coder_linear_speed
        self.logger = logger
        pass
    ...
    def update_odom(self, callback_speed:Callable[[float, float], None], position:OdomData, speed:OdomData, dt=0.0):
        ...
        elif self.started_moving_forward and not self.reached_wall and self.get_coder_linear_speed() <= self.epsilon_move_from_coder:
